...
class TRF():
	...
	def get_overlap(self,gff,dic,lens,key,genome,chip_seq):
		...
		for cen in centri:
			...
			f.write('>'+name+'\n'+chro_s[cen[0]:cen[1]]+'\n')
			f.close()
			self.plot(ax,cen[0],0,cen[1]-cen[0],110,self.colors.split(',')[1],0.8,False,'chro')

		ax.plot(x, index,alpha = .5,color = self.colors.split(',')[0])
		if self.chip_seq != 'None':
			for zsl in chip_seq[key]:
				self.plot(ax,zsl[0],-10,zsl[2],0,self.colors.split(',')[2],0.8,False,'chro')
	...
	def run(self):
		...
		import subprocess
		# 执行系统命令，不打印输出
		process = subprocess.Popen('%s %s 2 7 7 80 10 50 500 -f -d -h' % (self.trf_path,self.workpath+self.genome_file), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		# 获取命令的输出
		output, error = process.communicate()
		# 输出命令的输出
		# print("命令输出:", output.decode())
		files = os.listdir()
		# 遍历文件名，筛选出后缀为'.dat'的文件
		dat_files = [file for file in files if file.endswith('.dat')]
		print('Converting...')
		bez.TRF2gff(dat_files[0],self.trfgff,self.trffasta,self.workpath)
		print('Searching...')
